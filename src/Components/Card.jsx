...
const Card = (props) => {
  ...
  return (
  <div className="mx-2 w-full max-w-sm bg-white border border-gray-200 rounded-xl shadow-md dark:bg-gray-800 dark:border-gray-700">
    <Link to="">
        <img className="rounded-t-xl w-full h-56 object-contain" src={course.imageUrl} alt="product image" />
    </Link>
    ...
    <div className="px-2 pb-1">
        <Link to="">
            ...
            <h5 className="text-sm font-medium tracking-tight text-gray-900 dark:text-white flex gap-1 flex-wrap">
                {course.technologies.map((tech, index) => (
                    <li className='list-none'>{tech}</li>
                ))} 
            </h5>
        </Link>
        <p className='text-xl font-light text-gray-900'>Duration: {course.duration}</p> <span className='text-sm text-gray-900'>Level : {course.level}</span>
        ...
        <div className="flex items-center mt-2.5 mb-5">
            ...
            <p className='text-sm font-light text-gray-900'>Mode of Study : {course.mode} </p>
